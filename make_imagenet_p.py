...
class make_imagenet_p():
    def __init__(self, folder_path) -> None:
        self.folder_path = folder_path
        for index, folder in enumerate(sorted(os.listdir(self.folder_path))):
            for img_loc in os.listdir(self.folder_path + folder):
                img = trn.Resize(256)(PILImage.open(self.folder_path + folder + '/' + img_loc)).convert('RGB')
